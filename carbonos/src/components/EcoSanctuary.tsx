...
export default function EcoSanctuary({ 
  ...
  const handleNurture = async (itemId: string) => {
    ...
    if (targetItem.lastNurturedAt) {
      ...
      if (diffHrs < 0.1) { // 6 minutes cooldown for smooth demoing/testing
        alert("This module was nurtured very recently. Give it a few moments of breathing room to absorb atmospheric CO₂!");
        return;
      }
    }
  ...
  const handlePurchase = async (type: SanctuaryType) => {
    if (userPoints < type.cost) {
      alert("Insufficient Ledger XP balance! Complete Daily Logs and claims to accumulate points.");
      return;
    }
  ...
  // De-cultivate / Salvage (reclaims some XP as reward)
  const handleDeCultivate = async (item: SanctuaryItem) => {
    const type = SANCTUARY_TYPES.find(t => t.id === item.typeId);
    if (!type) return;

    const refundAmount = Math.round(type.cost * 0.3); // 30% XP refund
    const confirmed = window.confirm(`Are you sure you want to salvage ${item.name}? This will dismantle this carbon-reclaiming node, but yields a +${refundAmount} XP salvage allowance refund!`);
    if (!confirmed) return;
    ...
    if (onEarnPoints) {
      ...
      alert(`Salvaged! Earned back +${refundAmount} XP refund.`);
    }
